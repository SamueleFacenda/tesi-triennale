...
al raggiungimento di un'elevata interattivita: ogni interazione con l'interfaccia si riflette in un'interrogazione
...
\par Da questo spunto è nata una ricerca più approfondita che ha portato a questa tesi, ricerca posta a validare
...
Il prezzo da pagare per questa espressività ed estensività è il numero di triple che corrispondono a una singola nuvola, per circa 
...
a query di tipo OLTP, dove si va a ricercare tra singole risorse connesse, mentre query di tipo OLAP,
...
andando a misurare il tempo di caricamento e spazio su disco utilizzato dalle nuvole
...
le loro prestazioni riferite, è stato sempre usato
...
per un massimo di circa 20 secondi tra tutte le query su tutti i datasets. 
Tra questi troviamo anche il motore scelto per il visualizzatore di 3DOnt, andando a confermare
la scelta con una solida base analitica, mentre osserviamo che motori maturi ed estremamente popolari
possono avere risultati non dissimili da piccole librerie open-source. 
...
di applicazione tecnicamente irrealizzabili senza.
